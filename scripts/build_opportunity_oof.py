...
def main() -> None:
    ...
    if args.quotes and Path(args.quotes).exists():
        ...
        q = q.rename(columns={"prop": "stat"})
        q["game_id"] = pd.to_numeric(q["game_id"], errors="coerce")
        q["player_id"] = pd.to_numeric(q["player_id"], errors="coerce")
        quote_cut = (q[["game_id", "player_id", "pair_timestamp"]]
                     .rename(columns={"pair_timestamp": "quote_timestamp"})
                     .dropna().drop_duplicates(["game_id", "player_id"]))
    ...
    for fold_id, (vstart, vend) in enumerate(DEFAULT_FOLDS):
        ...
        pred = bundle.predict_active_pmfs(val, None, args.props)
        ...
        (ckpt_dir / f"opp_v2_fold_{fold_id:02d}_manifest.json").write_text(json.dumps({
