...
from wnba_props_model.sharp_v5.distribution import CountDistribution
from wnba_props_model.sharp_v6 import market_projection as MP


def _mix_atoms(lam, r, matoms, K):
    idx = np.where(matoms > 1e-4)[0]
    w = matoms[idx]; means = np.clip(lam * idx, 1e-6, None); k = np.arange(K + 1)
    if r is None:
        comp = poisson.pmf(k[:, None], means[None, :])
    else:
        p = r / (r + means); comp = nbinom.pmf(k[:, None], r, p[None, :])
    atoms = comp @ w; s = atoms.sum()
...
def main(date: str = typer.Option(None, "--date")) -> None:
    ...
    design_hash = json.loads((OUT / "V6_FREEZE_MANIFEST.json").read_text())["modeling_design_v6_sha256"]
    ...
    for i, row in slate.iterrows():
        ...
        for stat in V4C.TIER_A:
            ...
            a, ovf = _mix_atoms(lam_i, r, matoms[i], V4C.EMERGENCY_CAP[stat])   # vectorized mixture atoms
            kk = np.arange(a.size)
            ...
            for k, prob in enumerate(a):
                ...
                atom_rows.append({"game_id": gid, "canonical_player_id": pid, "player_name": pname,
                    ...
                    "prediction_timestamp": ts, "scheduled_tip": slate_date, "source_track": "PURE_PMF_V6",
                    "model_status": "TRAINED_PURE_UNCERTIFIED", "calibration_status": "PURE_UNCALIBRATED",
                    "tail_method": "minutes_mixture_nb2", "design_hash": design_hash[:16], "code_sha": code_sha[:12]})
            # moment-matched base for the tail-aware tilt projection
            r_mm = None if var <= mean else max(mean ** 2 / (var - mean), 0.3)
            base_dist = CountDistribution(max(mean, 1e-6), r_mm)
            mc_dist = None; proj_status = "NO_MARKET"
            cons = quotes.get((pid, stat))
            if cons and stat in MARKET_MAP.values():
                res = MP.project_multiline(base_dist, cons)
