...
def main(
    ...
    features: str = typer.Option("data/recovered_v2/modeling/wnba_pregame_features_t12.parquet", "--features"),
    ...
    # fail closed if bundle somehow tried to signal retrain
    if bundle.meta.get("retrain_in_daily"):
    ...
    try:
        ...
        raise SystemExit(f"BDL schedule fetch failed: {e}") from e
    ...
    try:
        ...
        avail = pd.DataFrame([
            {"player_id": r["player"]["id"], "status": r.get("status", "")} for r in inj if r.get("player")
        ])
    ...
    (out / "pricing_manifest.json").write_text(json.dumps(delivery.manifest, indent=2, default=str))

    # optional odds attach (diagnostic only — does not replace model_probability)
    quotes = _pull_quotes()
    (out / "market_quotes_diagnostic.json").write_text(json.dumps(quotes, indent=2, default=str))

    # prospective registry append-only
    _append_prospective(delivery, slate_date)

    typer.echo(
        f"V6 SLATE {slate_date}: games={len(games_live)} "
        f"players={delivery.manifest.get('n_players')} pmfs={delivery.manifest.get('n_pmfs')} "
        f"out={out}"
    )


def _pull_quotes() -> dict:
    ...
    try:
        ...
        for ev in events[:8]:
            r = requests.get(
                ...
                params={"apiKey": key, "regions": "us", "markets": "player_points,player_rebounds,player_assists,player_threes",
                        "oddsFormat": "american"},
                timeout=25,
            )
            out.append({"event": ev["id"], "status": r.status_code, "remaining": r.headers.get("x-requests-remaining")})
            time.sleep(0.1)
...
def _append_prospective(delivery, slate_date: str) -> None:
    import hashlib
    import numpy as np
    ...
    for (gid, pid, tgt), g in atoms.groupby(["game_id", "player_id", "target"]):
        ...
        pid_key = hashlib.sha256(f"v6:{gid}:{pid}:{tgt}:{slate_date}".encode()).hexdigest()[:24]
        rows.append({
            "prediction_id": pid_key, "forecast_timestamp": delivery.prediction_timestamp,
            "scheduled_tip": slate_date, "game_id": int(gid), "canonical_player_id": int(pid),
            "target": tgt, "atom_pmf_hash": h, "model_version": "wnba-sharp-pmf-v6", "settled": False,
        })
    new = pd.DataFrame(rows)
    if reg.exists():
        old = pd.read_parquet(reg)
        # immutable: never overwrite existing prediction_id
