...
def _merge_shooting(stats: pd.DataFrame, shooting: pd.DataFrame) -> pd.DataFrame:
    ...
    if shooting is not None and not shooting.empty:
        sl = (
            shooting[["game_id", "player_id", "fgm", "ftm"]]
...
def main() -> None:
    ...
    ap.add_argument(
        "--shooting-labels", default="data/recovered_v2/wnba_player_shooting_labels.parquet"
    )
    ap.add_argument(
        "--features",
        default="data/processed/wnba_player_game_features_wide.recovered_v2_20260725.parquet",
    )
    ...
    missing = [g for g in MISSING_GAME_IDS if int(g["game_id"]) not in present_final]
    ...
    if injury_path and injury_path.exists():
        ...
        try:
            ...
            _write_json(
                out / "INJURY_WORKBOOK_IDENTITY_AGGREGATE.json",
                {
    ...
    labels = build_participation_labels(
        stats, eligibility_evidence=evid if not evid.empty else None
    )
    ...
    if feature_path.exists():
        ...
        candidates = [
            ...
            if c.startswith(("player_minutes_", "player_pts_mean_"))
            or c in {"player_rest_days", "is_home", "team_rest_days"}
        ]
        # Exclude leakage
        assert_no_onset_leakage(candidates)
        feature_cols = [
            c for c in candidates if c not in {"actual_minutes", "minutes", "did_play"}
        ][:40]
        if feature_cols:
            ...
            cond_rows = len(cond)
            _write_json(
                out / "CONDITIONAL_MINUTES_TRAINING_AUDIT.json",
                {
                    "rows": cond_rows,
    ...
    confirmed_inactive_sources = (
        labels.loc[labels["participation_label_class"] == CONFIRMED_INACTIVE, "label_source"]
        .value_counts()
        .to_dict()
    )

    _write_json(
        out / "PARTICIPATION_LABEL_AUDIT.json",
        {
            "generated_at_utc": ts,
            "rows": len(labels),
    ...
    _write_json(
        out / "CANONICAL_REBUILD_AUDIT.json",
        {
            "generated_at_utc": ts,
            "raw_dir": args.raw_dir,
            "player_game_rows": len(stats),
            "seasons": sorted(int(s) for s in stats["season"].dropna().unique()),
            "shooting_label_rows": len(shooting_tbl),
            ...
            "model_fitted": False,
            "games_rows": len(games) if not games.empty else 0,
        },
    )

    # Aggregate injury leakage check artifact (no private rows)
    _write_json(
        out / "INJURY_LEAKAGE_CHECKS.json",
        {
    ...
    print(
        json.dumps(
            {
                "player_game_rows": len(stats),
                "shooting_rows": len(shooting_tbl),
                "participation_rows": len(labels),
