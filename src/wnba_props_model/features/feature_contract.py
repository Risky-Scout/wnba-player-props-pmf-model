...
# ---------------------------------------------------------------------------
# CERTIFIED feature-parity contract (owner phase 2). Zero tolerance: no benign
# absences, no optional dtype checks, no silent NaN/zero insertion, no forbidden
# feature, no duplicate, no order mutation, no schema-hash divergence. An artifact
# that expects a now-forbidden feature is REJECTED (retrain), never silently pruned.
# ---------------------------------------------------------------------------
...
def assert_certified_inference_parity(
    ...
    *,
    ...
    Requires: the artifact's exact ordered feature names all present (no missing, no benign
    tolerance); no duplicate feature name; no forbidden artifact feature (reject+retrain, never
    silently drop); exact dtype-kind map on every contract feature; no entirely-null required
    feature; exact schema hash (order is part of the contract -> catches order mutation); a
    recorded builder version. Extra frame columns are ignored ONLY after the full contract passes.
    """
    import pandas as pd  # local import to avoid module-level cycle
    usable = getattr(model, "_usable_cols", None)
    if not usable:
